...
def sga(month):
        ...
        try:
            ...
            if sheet_index < len(all_sheet_names):
                ...
                if target_column in csv_data.columns:
                    # Locate the row with "SG&A Consolidated"
                    sga_row = csv_data[csv_data.iloc[:, 0].str.contains('SG&A', case=False, na=False)]

                    if not sga_row.empty:
                        # Extract the value from the target column
                        sga_value = sga_row[target_column].values[0]
                        print(f'Value against "SG&A Consolidated" in "{target_column}": {sga_value}')

                        # Print all values in the row
                        print("\nValues in the 'SG&A Consolidated' row:")
                        sga_row_dict = sga_row.iloc[0].to_dict()
                        print(sga_row_dict)
                        ...
                        if budget_column in csv_data.columns and actual_column in csv_data.columns:
                            ...
                            budget_value = float(sga_row[budget_column].values[0])
                            actual_value = float(sga_row[actual_column].values[0])
                        ...
                        print(f'No row found with "SG&A Consolidated" in sheet "{all_sheet_names[sheet_index]}".')
                else:
    ...
    if isinstance(month, str) and month.lower() == 'all':
        for month_num in range(1, 10):  # January (1) to September (09)
...
sga(1)  # Processes Month
#sga('all')  # Processes all months from January to September
