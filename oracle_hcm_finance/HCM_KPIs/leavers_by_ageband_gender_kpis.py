...
def leavers_by_age_gender(month):
    ...
    try:
        ...
        dob_column = 'Date of Birth'  # Assuming there's a 'Date of Birth' column

        # Convert 'Actual Termination date' and 'Date of Birth' to datetime
        ...
        csv_data[dob_column] = pd.to_datetime(csv_data[dob_column], errors='coerce')

        # Calculate 'Age' from 'Date of Birth'
        current_date = datetime.now()
        csv_data['Age'] = (current_date - csv_data[dob_column]).dt.days // 365
        ...
        if month == 'all':
            # Iterate through each month (1 to 10)
            for m in range(1, 11):
