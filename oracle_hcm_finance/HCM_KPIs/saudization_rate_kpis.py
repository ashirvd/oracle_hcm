...
def saudization_filter(month):
    ...
    try:
        ...
        def calculate_saudization(month):
            # Create a cutoff date for the specified month and year (2024)
            cutoff_date = pd.Timestamp(f'{year}-{month:02d}-01')

            # Filter the data based on the specified conditions
            filtered_data = csv_data[
                (csv_data['Job title - English'] != 'Board Member') &  # Exclude 'Board Member' job title
                (csv_data['Actual Termination date'].isna() | (csv_data['Actual Termination date'] >= cutoff_date)) &  # Include active employees or those whose termination date is after the cutoff
                (csv_data['Nationality'].notnull()) &  # Include only rows where Nationality is not null
                (csv_data['Date of Joining'] < cutoff_date)  # Include employees who joined before the cutoff date
            ]
