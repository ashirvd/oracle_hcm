...
def length_of_service(month):
    ...
    def filter_by_termination_and_joining_date(data, target_year, month):
        ...
        if 'Actual Termination date' in data.columns and 'Date of Joining' in data.columns:
            ...
            start_date = pd.Timestamp(f'{target_year}-{month:02d}-01')
            end_date = start_date + pd.offsets.MonthEnd(1)

            # Filter active employees:
            # - Joining date must be before the end date
            # - Termination date must be after the end date or null
            filtered_data = data[
            ...
            # Additional check for necessary columns
            required_columns = ['Actual Termination date', 'Date of Joining']
            for col in required_columns:
                if col not in data.columns:
                    print(f'{col} column is not in the dataset.')
