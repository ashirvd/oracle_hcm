...
def variance_to_budget_fte(month_input):
    ...
    for month_index in month_indices:
            ...
            for col in range(1, excel_data.shape[1]):
                if (excel_data.iloc[10, col] == 'Budget' and  # Row 11 is index 10 in pandas
                    ...
                    try:
                        ...
                        total_budget_fte += budget_value
                    except ValueError:
                        print(f"Invalid budget value for {category_name} at row {row}, column {col}.")  # Debugging output
                        pass

            # Collecting Actual FTE for the category
            for col in range(1, excel_data.shape[1]):
                if (excel_data.iloc[10, col] == 'Actual' and  # Row 11 is index 10 in pandas
                    ...
                    try:
                        ...
                        total_actual_fte += actual_value
                    except ValueError:
                        print(f"Invalid actual value for {category_name} at row {row}, column {col}.")  # Debugging output
                        pass

            # Calculate variance for this category
            variance = total_actual_fte - total_budget_fte

            # Append the result for sorting later
            results.append((category_name, variance))  # Store without dividing
        ...
        for category_name, variance in results_sorted:
            print(f"{category_name:<30} {variance:>15.1f}")  # Adjusted display

# Example usage:
# Call the function for January (1)
variance_to_budget_fte(1)

# Call the function for all months
variance_to_budget_fte('all')
