...
def staff_cost(month_input):
    ...
    for month_index in month_indices:
        ...
        total_budget_headcount = total_actual_headcount = 0

        # Collecting Budget Data (Expense, FTE, and Headcount)
        for col in range(1, excel_data.shape[1]):
            if (excel_data.iloc[10, col] == 'Budget' and
                    ...
                    for idx, value in enumerate(column_values):
                        try:
                            ...
                            month_data.append({
                                ...
                                'Headcount Budget': 'N/A',  # Placeholder for Headcount Budget
                                'Headcount Actual': 'N/A'   # Placeholder for Headcount Actual
                            })
                ...
                elif excel_data.iloc[11, col] == 'Headcount':
                    # Process Headcount Budget
                    for idx, value in enumerate(column_values):
                        try:
                            budget_headcount = float(value)
                            if idx < len(month_data):
                                month_data[idx]['Headcount Budget'] = f"{budget_headcount:.0f}"
                            total_budget_headcount += budget_headcount
                        except ValueError:
                            pass

        # Collecting Actual Data (Expense, FTE, and Headcount)
        for col in range(1, excel_data.shape[1]):
            if (excel_data.iloc[10, col] == 'Actual' and
                ...
                if excel_data.iloc[11, col] == 'Employees Expenses':
                    ...
                    # Process Headcount Actual
                    for idx, value in enumerate(column_values):
                        try:
                            actual_headcount = float(value)
                            if idx < len(month_data):
                                month_data[idx]['Headcount Actual'] = f"{actual_headcount:.0f}"
                            total_actual_headcount += actual_headcount
                        except ValueError:
                            pass
        ...
        month_data.append({
            ...
            'Headcount Budget': f"{total_budget_headcount:.0f}",
            'Headcount Actual': f"{total_actual_headcount:.0f}"
        })
